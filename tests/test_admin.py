...
class TestAdmin:
    def test_create_item_success(self, auth_client):
        ...
        response = auth_client.post("/admin/items/", json=item_details)
    ...
    def test_create_duplicate_item_title_error(self, auth_client):
        ...
        auth_client.post("/admin/items/", json=item_details)

        response = auth_client.post("/admin/items/", json=duplicate_details)
        assert response.status_code == status.HTTP_409_CONFLICT

        body = response.json()
        assert body["detail"] == "Item already exists"

    def test_get_all_items_success(self, auth_client):
        """Verify the admin can retrieve a paginated list of all items."""
        auth_client.post("/admin/items/", json=create_item_data())

        response = auth_client.get("/admin/items?page=1&size=10")
    ...
    def test_get_all_users_success(self, auth_client):
        """Verify the admin can retrieve a paginated list of all registered users."""
        response = auth_client.get("/admin/users?page=1&size=10")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_delete_item_success(self, auth_client):
        ...
        response = auth_client.delete(f"/admin/items/{item_id}")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_deactivate_user_success(self, auth_client):
        """Modify user fields and verify changes."""
        users_resp = auth_client.get("/admin/users?page=1&size=1")
        user_id = users_resp.json()["data"][0]["id"]

        update_payload = {"is_active": False}
        response = auth_client.patch(f"/admin/users/{user_id}", json=update_payload)
    ...
    def test_update_user_not_found(self, auth_client):
        ...
        response = auth_client.patch("/admin/users/9999", json={"username": "Ghost"})
